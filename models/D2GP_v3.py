...
class BidirectionalCrossAttention(nn.Module):
    def __init__(self, embed_dim1=512, embed_dim2=256, num_heads=4):
        super().__init__()
        self.proj_mat2_to_1 = nn.Linear(embed_dim2, embed_dim1)
        self.cross_attn_1to2 = nn.MultiheadAttention(embed_dim1, num_heads)
        self.proj_mat1_to_2 = nn.Linear(embed_dim1, embed_dim2)
        self.cross_attn_2to1 = nn.MultiheadAttention(embed_dim2, num_heads)
        self.fuse = nn.Linear(embed_dim1 + embed_dim1, embed_dim1)

    def forward(self, mat1, mat2):
        B, C1, H1, W1 = mat1.shape
        B, C2, H2, W2 = mat2.shape
        
        # Flatten spatial dimensions
        x1 = mat1.view(B, C1, -1).permute(2, 0, 1)  # [L1, B, C1] = [1024, 4, 512]
        x2 = mat2.view(B, C2, -1).permute(2, 0, 1)  # [L2, B, C2] = [4096, 4, 256]

        # Branch 1: mat1 attends to mat2
        x2_proj = self.proj_mat2_to_1(x2)  # [4096, 4, 512]
        out1, _ = self.cross_attn_1to2(x1, x2_proj, x2_proj)  # [1024, 4, 512]

        # Branch 2: mat2 attends to mat1
        x1_proj = self.proj_mat1_to_2(x1)  # [1024, 4, 256]
        out2, _ = self.cross_attn_2to1(x2, x1_proj, x1_proj)  # [4096, 4, 256]
        out2_up = self.proj_mat2_to_1(out2)  # [4096, 4, 512]

        # Resize to mat1's spatial dimensions
        out2_up = out2_up.permute(1, 2, 0).view(B, C1, H2, W2)  # [4, 512, 64, 64]
        out2_up = F.interpolate(out2_up, size=(H1, W1), mode='bilinear')  # [4, 512, 32, 32]
        out2_up = out2_up.view(B, C1, -1).permute(2, 0, 1)  # [1024, 4, 512]

        # Concatenate and fuse
        out_cat = torch.cat([out1, out2_up], dim=-1)  # [1024, 4, 1024]
        fused = self.fuse(out_cat)  # [1024, 4, 512]
        
        return fused.permute(1, 2, 0).view(B, C1, H1, W1)  # [4, 512, 32, 32]
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        self.fuse_model = BidirectionalCrossAttention()  # try num_head = 2 and 4
    ...
    def forward(self, image, depth, prototypes):
        ...
        out = self.context_module(out) # [1, 128, 32, 32]

        out = F.interpolate(out, size=(16, 16), mode='area') # [1, 128, 16, 16]
