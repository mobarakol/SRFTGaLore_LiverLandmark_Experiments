...
from peft import AdaLoraConfig  # 加入 AdaLoRA config
...
def apply_adalora_to_sam2_encoder(sam_encoder, r=8, alpha=1.0, total_step=1000):
    ...
    sam_encoder = get_peft_model(sam_encoder, adalora_config)

    # freeze base parameters, keep adapters trainable
    for name, param in sam_encoder.named_parameters():
        if "lora_" not in name and "adalora_" not in name:
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        # 4. Apply AdaLoRA to encoder
        rank = 4
        total_step = 10 * (921 // 2)  # epoch * (train_size // batch_size)
        sam_encoder = apply_adalora_to_sam2_encoder(sam2_encoder, r=rank, alpha=16, total_step=total_step)
