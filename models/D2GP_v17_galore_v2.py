...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        # ---------- SAM1 decoder ----------
        sam = sam_model_registry["vit_b"](checkpoint="sam_path/sam_vit_b_01ec64.pth")
        self.mask_decoder = sam.mask_decoder
        self.prompt_encoder = sam.prompt_encoder

        self.mask_head = nn.Conv2d(1, 4, kernel_size=1)
        # -----------------------------------
    ...
    def forward(self,
        ...
        B, C
# ------- Calling SAM1 decoder -------
        B, C, H, W = fused_feat.shape
        print("fused_feat shape:", fused_feat.shape)  # [2, 256, 64, 64]
        device = fused_feat.device

        image_embeddings = fused_feat

        image_pe = self.prompt_encoder.get_dense_pe().to(image_embeddings.device)
        ...
        # print("masks shape:", masks.shape) # [2, 1, 256, 256]
        logits = self.mask_head(masks)  # masks shape: [B, 1, 256, 256] → [B, 4, 256, 256]
        print("logits shape:", logits.shape)
        return logits, depth_feat
