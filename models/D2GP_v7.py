# Geometry Augmentation -> Cross Attention Fusion
import math
import warnings
import torch
...
class DPE(nn.Module):
    ...
    def forward(self, features, sam_feature, prototypes):
        ...
        sam_feat = F.interpolate(sam_feature, size=16, mode='bilinear', align_corners=False).reshape(sam_feature.shape[0], -1,
                                                                                16 * 16)  # 2, 256, 256
...
class CrossAttentionFuse(nn.Module):
    ...
    def forward(self, mat1, mat2):
        ...
        return fused  # Fused feature map with shape [B, 512, 32, 32]
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        self.cross_attention_fuse = CrossAttentionFuse(embed_dim1=128, embed_dim2=256, num_heads=4)

        self.edge_head = nn.Sequential(nn.Conv2d(128, 3, kernel_size=1))
    ...
    def forward(self, image, depth, prototypes):
        ...
        # Segmentic-specific Geometry Augmentation
        # fused_feat, edge_out, geometry_feat = self.prompt_fusion_layer(out, original_depth, prototypes)
        geometry_feat = F.interpolate(original_depth, size=(32, 32), mode='bilinear', align_corners=False)
        fused_feat = self.cross_attention_fuse(out, geometry_feat)
        # after fused_feat = self.cross_attention_fuse(...)
        edge_out = self.edge_head(fused_feat)
