...
def apply_mora_to_sam2_encoder(sam_encoder, r=8, alpha=1.0):
    ...
    sam_encoder = get_peft_model(sam_encoder, mora_config)

    # freeze base parameters, keep adapters trainable
    for name, param in sam_encoder.named_parameters():
        if "lora_" not in name and "mora_" not in name:
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        # 4. Apply MoRA to encoder
        rank = 6
        sam_encoder = apply_mora_to_sam2_encoder(sam2_encoder, r=rank, alpha=16)

        # 5. Assign to self.sam2_encoder
        self.sam2_encoder = sam_encoder
        print("MoRA applied to SAM2 encoder. Rank =", rank)
