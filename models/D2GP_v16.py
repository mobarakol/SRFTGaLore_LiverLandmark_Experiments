# Replace SAM2+DA2 -> CA -> SAM2 decoder
import math
import warnings
import torch
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        # 添加用於調整 high_res_features 通道數的卷積層
        # SAM2 decoder 期望：feat_s0 (32 channels), feat_s1 (64 channels)
        self.conv_s0 = nn.Conv2d(256, 32, kernel_size=1, stride=1)  # 對應最高解析度特徵
        self.conv_s1 = nn.Conv2d(256, 64, kernel_size=1, stride=1)  # 對應中等解析度特徵
        
        # 添加最終的分類層，將 SAM2 的單通道輸出轉換為多類別輸出
        self.final_classifier = nn.Conv2d(1, num_classes, kernel_size=1)
    ...
    def forward(self,
        ...
        with torch.no_grad():
            images_list = [img.cpu().permute(1, 2, 0).numpy() for img in image]
            inputs = self.da2_processor(images=images_list, return_tensors="pt", do_rescale=False)
        ...
        #     sparse_prompt_embeddings = torch.zeros(B, 0, 256, device=device) 

        # if dense_prompt_embeddings is None:
        #     dense_prompt_embeddings = torch.zeros(B, 256, H, W, device=device) 

        # If no user prompt, use default dummy embeddings from prompt encoder
        if sparse_prompt_embeddings is None or dense_prompt_embeddings is None:
            B = image.shape[0]
            dummy_points = (
                torch.zeros((B, 1, 2), device=image.device),      # coordinates
                torch.ones((B, 1), device=image.device) * -1      # labels
            )
        ...
        # set high_res_features - SAM2 decoder expect two different resolutions
        # SAM2 decoder expect channels：feat_s0-> 32, feat_s1-> 64
        
        # skip3: [B, 256, 256, 256], skip2: [B, 256, 128, 128]
        feat_s0 = self.conv_s0(F.interpolate(skip3, size=(H*4, W*4), mode='bilinear', align_corners=False))  # [B, 32, H*4, W*4]
        feat_s1 = self.conv_s1(F.interpolate(skip2, size=(H*2, W*2), mode='bilinear', align_corners=False))  # [B, 64, H*2, W*2]
        high_res_features = [feat_s0, feat_s1]
        
        outs, iou_pred, sam_tokens_out, object_score_logits = self.decoder(
            image_embeddings=fused_feat,
            image_pe=image_pe,
            sparse_prompt_embeddings=sparse_prompt_embeddings,  # Use the properly sized tensor
            dense_prompt_embeddings=dense_prompt_embeddings,    # Use the properly sized tensor
        ...
        # print('outs shapes before decoder:', [out.shape for out in outs])  # 

        # outs, out_visual = self.decoder(enc_outs=outs) # outs: [4, 1024, 1024]
        # print('decoder outs shape:', outs.shape) # [4, 4, 256, 256]
        
        # 將 SAM2 的單通道輸出轉換為多類別輸出
        # print('outs shape before final classifier:', outs.shape)  # [B, 1, H, W]
        outs = self.final_classifier(outs)  # [B, num_classes, H, W]
        outs = F.log_softmax(outs, dim=1)  # 應用 log_softmax 到類別維度
        # print('Final output shape:', outs.shape)
