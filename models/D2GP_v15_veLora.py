...
def apply_velora_to_sam2_encoder(sam_encoder, r=16, alpha=32):
    ...
    sam_encoder = get_peft_model(sam_encoder, velora_config)

    # freeze base parameters, keep adapters trainable
    for name, param in sam_encoder.named_parameters():
        if "lora_" not in name and "velora_" not in name:
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        rank = 16
        sam_encoder = apply_velora_to_sam2_encoder(sam2_encoder, r=rank, alpha=32)
