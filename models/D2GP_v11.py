# D2GP using Cross Attention, SAM encoder without LoRA for depth, and with LoRA for image
import math
import warnings
import torch
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        sam_depth = sam_model_registry["vit_b"](
            checkpoint="sam_path/sam_vit_b_01ec64.pth")

        self.sam_encoder_depth = sam_depth.image_encoder

        for param in self.sam_encoder_depth.parameters():
    ...
    # def forward(self, image, depth, prototypes):
        ...
        original_depth = self.sam_encoder_depth(original_depth) # [1, 256, 64, 64]
