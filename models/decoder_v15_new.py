...
class DecoderModule(nn.Module):
    def __init__(self,
        ...
        self.side_output = nn.Conv2d(channels_dec,
                                     num_classes,
                                     kernel_size=1)
        if encoder_decoder_fusion == 'concat':
            self.fusion_conv = ConvBNAct(channels_dec * 2, channels_dec, kernel_size=1, activation=activation)
    ...
    def forward(self, decoder_features, encoder_features):
        ...
        if encoder_features is not None:
            ...
            elif self.encoder_decoder_fusion == 'concat':
                out = torch.cat([out, encoder_features], dim=1)
