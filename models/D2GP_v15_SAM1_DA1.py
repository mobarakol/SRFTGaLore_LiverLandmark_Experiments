...
def get_last_n_block_outputs(sam_encoder, image):
    out = sam_encoder(image)  # out: [B, C, H, W]
    
    # 如果你沒有 skip1/2/3，這裡可以全部設成 out，或設成 None
    skip1 = out
    skip2 = out
    skip3 = out
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        # -------------- Add SAM1 encoder --------------
        sam1 = sam_model_registry["vit_b"](
            checkpoint="sam_path/sam_vit_b_01ec64.pth")

        self.sam1_encoder = sam1.image_encoder

        for param in self.sam1_encoder.parameters():
        ...
        # -------------- Depth Anything 1 Encoder --------------
        model_name = "LiheYoung/depth-anything-small-hf"

        # initialize DA1 encoder
        self.da1_processor = AutoImageProcessor.from_pretrained(model_name)
        da1_model = DepthAnythingForDepthEstimation.from_pretrained(model_name)
        self.da1_encoder = da1_model.backbone  # 正確引用 encoder
        # freeze the DA1 encoder
        for param in self.da1_encoder.parameters():
            param.requires_grad = False
    ...
    def forward(self, image, depth):
        ...
        # ----- Encoder: DA1 for depth ------
        with torch.no_grad():
            inputs = self.da1_processor(images=image, return_tensors="pt", do_rescale=False)
            # features = self.da1_encoder(inputs["pixel_values"])
            device = next(self.da1_encoder.parameters()).device
            pixel_values = inputs["pixel_values"].to(device)
            features = self.da1_encoder(pixel_values)
            # print("DA1 encoder output type:", type(features))
        ...
        # ----- Encoder: SAM for RGB ------
        out, skip1, skip2, skip3  = get_last_n_block_outputs(self.sam1_encoder, image)# [4, 64, 64, 768]
