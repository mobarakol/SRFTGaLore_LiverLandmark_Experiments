...
class GatedCrossAttention(nn.Module):
    def __init__(self, embed_dim1=512, embed_dim2=256, num_heads=2):
        super().__init__()
        self.proj_mat2 = nn.Linear(embed_dim2, embed_dim1)
        self.cross_attn = nn.MultiheadAttention(embed_dim1, num_heads)
        self.gate = nn.Parameter(torch.tensor(0.5))  # start in the middle

    def forward(self, mat1, mat2):
        B, C1, H, W = mat1.shape
        B, C2, _, _ = mat2.shape
        # Flatten spatial dims and permute for MHA: [seq, batch, channels]
        x1 = mat1.view(B, C1, -1).permute(2, 0, 1)  # [HW, B, 512]
        x2 = mat2.view(B, C2, -1).permute(2, 0, 1)  # [HW, B, 256]
        x2_proj = self.proj_mat2(x2)                # [HW, B, 512]
        attn_out, _ = self.cross_attn(query=x1, key=x2_proj, value=x2_proj)
        # Gated fusion
        fused = self.gate * attn_out + (1 - self.gate) * x1
        # Back to [B, 512, 32, 32]
        return fused.permute(1, 2, 0).contiguous().view(B, C1, H, W)
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        self.fuse_model = GatedCrossAttention()  # try num_head = 2 and 4
