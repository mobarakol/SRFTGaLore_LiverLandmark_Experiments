# original CA (v_1) with SAM+LORA
# train file: train_attention.py (remember to change the save_path)
import math
import warnings
import torch
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        self.fuse_model = CrossAttentionFuse(embed_dim1=512, embed_dim2=256, num_heads=2)  # try num_head = 2 and 4
    ...
    def forward(self, image, depth, prototypes):
        ...
        # 1024×1024 depth → first_conv → SAM 编码 → original_depth:[1,256,64,64]
        depth_feat = F.interpolate(original_depth, size=(32, 32), mode='area')
        # out = self.mid_conv(torch.cat((out, depth_feat), dim=1))
        out = self.fuse_model(out, depth_feat)
