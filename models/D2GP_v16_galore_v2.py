...
# def apply_galore_to_sam2_encoder(sam2_encoder, r=6, alpha=16):
#     target_modules_list = ["attn.qkv", "attn.proj"]
#     galore_params = []
#     id_galore_params = []

#      # Freeze all non-target modules in SAM encoder, and collect all trainable GaLore params
...
#     regular_params = [p for p in sam2_encoder.parameters() if p.requires_grad and id(p) not in id_galore_params]


#     param_groups = [
#         {'params': regular_params},
#         {'params': galore_params, 'rank': r, 'update_proj_gap': 50, 'scale': 1, 'proj_type': "std"}
#     ]

#     # 初始化 GaLore optimizer
#     optimizer = GaLoreAdamW(param_groups, lr=1e-5, weight_decay=0.0)

#     # 印出總訓練參數量（可選）
#     total_trainable_params = sum(p.numel() for p in sam2_encoder.parameters() if p.requires_grad)
#     print(f"✅ GaLore applied to SAM2 encoder. Total trainable parameters: {total_trainable_params / 1e6:.2f}M")

#     return sam2_encoder, optimizer
...
class D2GPLand(nn.Module):
    def __init__(self,
        ...
        for module_name, module in sam2_encoder.named_modules():
            if isinstance(module, nn.Linear) and any(t in module_name for t in target_modules_list):
                ...
                if hasattr(module, "bias") and module.bias is not None:
                    module.bias.requires_grad = False  # 冻结 bias
            else:
                for param in module.parameters():
                    param.requires_grad = False

        # Collect other trainable (regular) parameters from entire model
        # We'll delay this until after full model is built
        ...
        # print("GaLore applied to SAM2 encoder. Rank =", rank)
        ...
        # Replace decoder with pretrained SAM2 mask decoder
        sam2_full_model = build_sam2(model_cfg, checkpoint, device="cuda")
        self.sam2_mask_decoder = sam2_full_model.sam_mask_decoder  # pretrained mask decoder

        # Output projection for 4-class segmentation
        self.sam2_output_proj = nn.Conv2d(1, num_classes, kernel_size=1)
    ...
    def get_galore_optimizer(self, lr=1e-5, weight_decay=0.0, rank=128):
        regular_params = [
    ...
    def forward(self, image, depth):
        ...
        with torch.no_grad():
            ...
            inputs = self.da2_processor(images=list(image.cpu()), return_tensors="pt", do_rescale=False)
        ...
        # SAM2 decoder forward - create all dummy variables
        B, C, H, W = fused_feat.shape
        dummy_sparse_prompt = torch.zeros((B, 0, 256), device=fused_feat.device)
        dummy_dense_prompt = torch.zeros((B, 256, 64, 64), device=fused_feat.device)
        dummy_pos_enc = torch.zeros((B, 4, 64, 64), device=fused_feat.device)
        
        assert fused_feat.shape[0] == dummy_dense_prompt.shape[0], "Batch size mismatch in decoder input!"
        # print('Fused feature shape:', fused_feat.shape)  # [4, 256, 64, 64]

        masks, iou_pred = self.sam2_mask_decoder(
            image_embeddings=fused_feat,
            image_pe=dummy_pos_enc,
            sparse_prompt_embeddings=dummy_sparse_prompt,
            dense_prompt_embeddings=dummy_dense_prompt,
            multimask_output=False,
            repeat_image=True,
        )

        outs = self.sam2_output_proj(masks)  # Convert to [B, 4, 256, 256]
        outs = F.log_softmax(outs, dim=1)
        out_visual = None  # placeholder to match return signature
