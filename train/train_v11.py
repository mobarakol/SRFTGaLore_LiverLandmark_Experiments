...
from models.D2GP_v11 import D2GPLand, Edge_Prototypes


print ("D2GP using Cross Attention, SAM encoder without LoRA for depth, and with LoRA for image") 
...
def main(save_path, args):
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, (X_batch, depth, y_batch, *rest) in tqdm(enumerate(train_loader)):
            ...
            # edge_gt = F.interpolate(y_batch[:, 1:, :, :], size=16, mode="bilinear", align_corners=True)
        ...
        if np.mean(mDice) > best_dice:
            ...
            torch.save(model.state_dict(), save_path + "best_model_path_v11.pth")
            # torch.save(edge_prototypes_model.state_dict(), save_path + "best_prototype_path_v9.pth")
            print("save best model at", save_path + "best_model_path_v11.pth")
            # print("save best prototype at", save_path + "best_prototype_path_v9.pth")
