...
from models.D2GP_v15_galore_v3 import D2GPLand, Edge_Prototypes


print ("v15 galore energy, lr: 1e-5") 
...
def main(save_path, args):
    ...
    #     if p.requires_grad:
    #         print(f"[Trainable] {name} — shape: {p.shape}")
    ...
    for epoch in range(args.epoch):
        ...
        # print("[GPU] Memory allocated:", torch.cuda.memory_allocated(device) / (1024 ** 2), "MB")
        # print("[GPU] Memory reserved (cache):", torch.cuda.memory_reserved(device) / (1024 ** 2), "MB")
        # print("[GPU] Max memory allocated:", torch.cuda.max_memory_allocated(device) / (1024 ** 2), "MB")
        # print("[GPU] Max memory reserved:", torch.cuda.max_memory_reserved(device) / (1024 ** 2), "MB")
        if np.mean(mDice) > best_dice:
            best_dice = np.mean(mDice)
            torch.save(model.state_dict(), save_path + "best_model_path_v15_galore_v3_no.pth")
            print("save best model at", save_path + "best_model_path_v15_galore_v3_no.pth")
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--decay_lr', type=float, default=5e-7)
