...
from utils.dataset_aug import LandmarkDataset
from models.D2GP_v14_aug import D2GPLand, Edge_Prototypes


print ("Replace the depth using DA2 encoder based on v_10, change CA, add color augmentation with tensor(not PIL)") 
...
def main(save_path, args):
    ...
    train_transform = T.Compose([
        T.ToTensor(),
        # T.RandomResizedCrop(1024, scale=(0.9, 1.1)),
    ...
    print(f'Data Augmentation - Brightness: {args.brightness}, Contrast: {args.contrast}, Saturation: {args.saturation}')
    ...
    for epoch in range(args.epoch):
        ...
        if np.mean(mDice) > best_dice:
            ...
            torch.save(model.state_dict(), save_path + "best_model_path_v14_color_tensor.pth")
            print("save best model at", save_path + "best_model_path_v14_color_tensor.pth")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--brightness', type=float, default=0.08, help='ColorJitter brightness')
    parser.add_argument('--contrast', type=float, default=0.08, help='ColorJitter contrast')
    parser.add_argument('--saturation', type=float, default=0.08, help='ColorJitter saturation')
    parser.add_argument('--use_tensor_aug', action='store_true', help='Use tensor-based color augmentation')
