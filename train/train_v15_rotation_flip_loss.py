...
from models.D2GP_v15_CA_v2_BCR import D2GPLand, Edge_Prototypes
...
def main(save_path, args):
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            for X_batch, depth, y_batch, _, _, _, name in tqdm(val_loader):
                X_batch = X_batch.to(device)
                y_batch = y_batch.to(device)
                depth = depth.to(device)
        ...
        if np.mean(mDice) > best_dice:
            ...
            torch.save(model.state_dict(), save_path + "best_model_path_v15_loss.pth")
            print("save best model at", save_path + "best_model_path_v15_loss.pth")
