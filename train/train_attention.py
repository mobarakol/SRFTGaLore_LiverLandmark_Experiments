...
from models.D2GP_v1_lora import D2GPLand, Edge_Prototypes

print ("D2GP_v1_lora, lr 1e-4")
# print ("v_12: original CA (v_1) with SAM+LORA") 
...
def main(save_path, args):
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, (X_batch, depth, y_batch, *rest) in tqdm(enumerate(train_loader)):
            ...
            edge_gt = F.interpolate(y_batch[:, 1:, :, :], size=16, mode="bilinear", align_corners=True) # edge_gt shape: torch.Size([4, 3, 16, 16])
        ...
        if np.mean(mDice) > best_dice:
            ...
            torch.save(model.state_dict(), save_path + "best_model_path_D2GPLand_lora_1e_4.pth")
            torch.save(edge_prototypes_model.state_dict(), save_path + "best_prototype_path_D2GPLand_lora_1e_4.pth")
            print("save best model at", save_path + "best_model_path_D2GPLand_lora_1e_4.pth")
            print("save best prototype at", save_path + "best_prototype_path_D2GPLand_lora_1e_4.pth")
