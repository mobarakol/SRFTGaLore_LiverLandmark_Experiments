...
from models.D2GP_v15_dora import D2GPLand, Edge_Prototypes


print ("v15 dora, CA v1, add rotation and flip, lr: 5e-5") 
...
def main(save_path, args):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=10, factor=0.85)
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            for X_batch, depth, y_batch, _ in tqdm(val_loader):
                X_batch = X_batch.to(device)
                y_batch = y_batch.to(device)
                depth = depth.to(device)
        ...
        if np.mean(mDice) > best_dice:
            ...
            torch.save(model.state_dict(), save_path + "best_model_path_v15_dora_lr_decay.pth")
            print("save best model at", save_path + "best_model_path_v15_dora_lr_decay.pth")
