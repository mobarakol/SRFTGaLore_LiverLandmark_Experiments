...
from models.D2GP_v18 import D2GPLand, Edge_Prototypes

print ("v18 galore uniform, SAM2->decoder, lr: 5e-5") 
...
def main(save_path, args):
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            for X_batch, depth, y_batch, *rest in tqdm(val_loader):
                X_batch = X_batch.to(device)
                y_batch = y_batch.to(device)
        ...
        if np.mean(mDice) > best_dice:
            ...
            torch.save(model.state_dict(), save_path + "best_model_path_v18_r256.pth")
            print("save best model at", save_path + "best_model_path_v18_r256.pth")
